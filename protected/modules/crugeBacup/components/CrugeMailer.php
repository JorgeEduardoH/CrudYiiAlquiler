...
class CrugeMailer extends CrugeMailerBase implements ICrugeMailer
{
    ...
    */
    ...
    // enviar correo de redencion
    public function notificarStockMercancia($usuario,$asunto,$id_redimir) {

     $this->sendEmail(
       $usuario->email,$asunto, 
       $this->render('application.components.vista_stockvence',array('data'=>$usuario, 'id_redimir'=>$id_redimir))
     );
  }
  public function mail_contacto($destino,$asunto,$subject,$body,$headers) {
// clubk@kia.com.co
     $this->sendEmail(
       $destino,$asunto, 
       $this->render('application.components.vista_contacto',array( 'subject'=>$subject, 'body'=>$body,'headers'=>$headers ))
     );
